...
local function carregaPontos ()
	-- Verificar se o arquivo existe abrindo-o somente para leitura
	local pasta = io.open (filePath, "r") -- "r" == somente para leitura

	if pasta then
		local contents = pasta:read ("*a") -- Extrai os dados do arquivo e guarda na variável contents (formato JSON)
		io.close (pasta) -- fecha o arquivo pontos.json
		pontosTable = json.decode (contents) -- Decodificar a variável contents de json para lua e armanená-los na tabela.
	end
...
function scene:create( event )
	...
	table.sort (pontosTable, compare) -- Classifica a ordem definida na function para a pontosTable
